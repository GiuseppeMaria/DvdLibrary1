...
public class Elimina extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            request.setAttribute("Errori",errMsg);
            errMsg.add("The id selected is not valid");
            RequestDispatcher view=request.getRequestDispatcher("error_page.view");
            view.forward(request, response);
        }
        
            for (int i=0;i<instance.Libreria.size();i++){
                DvdItem dvd = instance.Libreria.get(i);
                if (dvd.getId() == id){
                    instance.Libreria.remove(i);
                    RequestDispatcher view = request.getRequestDispatcher("show_dvd.do");
                    view.forward(request, response);
                }
